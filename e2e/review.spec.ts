...
test("approves a template and deduplicates regeneration requests", async ({
  ...
  await expect(page.getByText("848", { exact: true })).toBeVisible();
  ...
  await expect(
    difficultyTable.getByRole("row", { name: /Foundational 80/ }),
  ).toBeVisible();
  await expect(
    difficultyTable.getByRole("row", { name: /Advanced 80/ }),
  ).toBeVisible();
  await expect(
