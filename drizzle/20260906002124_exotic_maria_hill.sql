ALTER TABLE "generation_runs" DROP CONSTRAINT "generation_run_completion_check";--> statement-breakpoint
ALTER TABLE "generation_runs" ADD COLUMN "claim_token" varchar(128);--> statement-breakpoint
ALTER TABLE "generation_runs" ADD COLUMN "claimed_by" varchar(160);--> statement-breakpoint
...
ALTER TABLE "generation_runs" ADD COLUMN "attempt_count" integer DEFAULT 0 NOT NULL;--> statement-breakpoint
CREATE INDEX "generation_run_queue_idx" ON "generation_runs" USING btree ("status","lease_expires_at","started_at");--> statement-breakpoint
ALTER TABLE "generation_runs" ADD CONSTRAINT "generation_run_lease_check" CHECK ("generation_runs"."attempt_count" >= 0 AND (
        ("generation_runs"."status" = 'PENDING' AND "generation_runs"."attempt_count" = 0 AND "generation_runs"."claim_token" IS NULL AND "generation_runs"."claimed_by" IS NULL AND "generation_runs"."lease_expires_at" IS NULL AND "generation_runs"."last_heartbeat_at" IS NULL)
        OR ("generation_runs"."status" = 'RUNNING' AND "generation_runs"."attempt_count" > 0 AND "generation_runs"."claim_token" IS NOT NULL AND "generation_runs"."claimed_by" IS NOT NULL AND "generation_runs"."lease_expires_at" IS NOT NULL AND "generation_runs"."last_heartbeat_at" IS NOT NULL AND "generation_runs"."lease_expires_at" > "generation_runs"."last_heartbeat_at")
        OR ("generation_runs"."status" NOT IN ('PENDING', 'RUNNING') AND (("generation_runs"."attempt_count" = 0 AND "generation_runs"."claim_token" IS NULL AND "generation_runs"."claimed_by" IS NULL AND "generation_runs"."lease_expires_at" IS NULL AND "generation_runs"."last_heartbeat_at" IS NULL) OR ("generation_runs"."attempt_count" > 0 AND "generation_runs"."claim_token" IS NOT NULL AND "generation_runs"."claimed_by" IS NOT NULL AND "generation_runs"."lease_expires_at" IS NOT NULL AND "generation_runs"."last_heartbeat_at" IS NOT NULL)))
      ));--> statement-breakpoint
ALTER TABLE "generation_runs" ADD CONSTRAINT "generation_run_completion_check" CHECK (("generation_runs"."status" IN ('PENDING', 'RUNNING') AND "generation_runs"."completed_at" IS NULL) OR ("generation_runs"."status" NOT IN ('PENDING', 'RUNNING') AND "generation_runs"."completed_at" IS NOT NULL));--> statement-breakpoint
  ...
  IF OLD.status = 'PENDING' THEN
    ...
    IF NEW.status = 'CANCELLED' THEN
      IF NEW.provider IS DISTINCT FROM OLD.provider
        ...
        RAISE EXCEPTION 'pending cancellation may not add execution results'
          USING ERRCODE = '55000';
      END IF;
      RETURN NEW;
    END IF;
