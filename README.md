...
[![AWS mirror](https://img.shields.io/badge/AWS_Mirror-Lambda-ff9900.svg)](https://mq6xnisbdyio5lcyhi6afmv7ti0lfomm.lambda-url.us-east-1.on.aws)
...
| AWS deployment                              | Public scale-to-zero Lambda/ECR mirror deployed by Terraform; health, Google OAuth, protected practice, SSM/IAM, and logs verified |
...
- **Production-shaped delivery:** isolated browser-test databases, transactional failure tests, a non-root distroless standalone container, one-shot migrations, health checks, and a digest-pinned scale-to-zero AWS Lambda deployment managed by Terraform.
...
| [Deployment](docs/DEPLOYMENT.md)                           | Vercel/Neon MVP, Docker operations, live Lambda mirror, and commercial AWS architecture  |
...
To exercise the production-style image locally instead, run `docker compose --profile application up --build`. This starts PostgreSQL, runs migrations once, and serves a non-root standalone Next.js container. See [Deployment and container operations](docs/DEPLOYMENT.md) for port overrides, health checks, runtime configuration, the live Lambda mirror, and the unapplied commercial AWS plan.
...
The same reviewed commit is also live through a [scale-to-zero AWS mirror](https://mq6xnisbdyio5lcyhi6afmv7ti0lfomm.lambda-url.us-east-1.on.aws). The Next.js 16 standalone output runs as an unprivileged user in a digest-pinned distroless Lambda container through AWS's Lambda Web Adapter, reads one encrypted SSM runtime parameter through exact-resource IAM, and uses a generated Function URL. The release image's September 14, 2026 ECR enhanced scan completed with no reported findings; future scans can change as vulnerability data evolves. ECR retains only three images, Lambda has no provisioned concurrency, CloudWatch logs expire after seven days, and an alarm watches Lambda errors. The account's regional concurrency quota of 10 is the current hard ceiling because AWS requires all 10 executions to remain unreserved on this new account. See [`infra/serverless`](infra/serverless) for the reproducible two-pass procedure and cost caveats.
...
The standalone application image, Lambda-compatible image, migration job, Compose topology, database-aware health endpoint, and both Terraform configurations are implemented and validated. The scale-to-zero root is applied and deliberately retains Neon while omitting the always-on load balancer, NAT, Fargate, and RDS resources. The separate commercial infrastructure root enforces digest-pinned images, private tasks and database subnets, production availability guards, secret injection, alarms, and an account-wide budget, but that higher-cost stack remains unapplied. See [deployment operations](docs/DEPLOYMENT.md).
