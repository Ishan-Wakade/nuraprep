...
The cookie policy is explicit application configuration rather than an undocumented library default. Unit tests pass it through the installed Better Auth cookie builder and verify that production uses the `__Secure-` prefix, `Secure`, `HttpOnly`, `SameSite=Lax`, path `/`, and no `Domain` attribute. Local HTTP development omits only `Secure` and the secure prefix. A staging test must still inspect an actual browser cookie over TLS before launch.
...
Client-IP trust is platform-specific. With `DEPLOYMENT_PLATFORM=VERCEL`, Better Auth reads Vercel's platform-owned `X-Vercel-Forwarded-For` header, which Vercel overwrites before the request reaches the application. AWS and other self-hosted targets instead interpret `X-Forwarded-For` through an explicit `TRUSTED_PROXY_CIDRS` allowlist. The AWS target fixes the ALB to append mode, disables client-port inclusion, restricts ECS ingress to the ALB security group, and supplies only the two ALB subnet CIDRs. Production startup fails if self-hosted proxy configuration is empty or malformed; broad values such as `0.0.0.0/0` are prohibited. Forwarded host/protocol trust remains off because the canonical base URL is configured directly.
...
- Production startup fails if Google credentials, auth and Server Action secrets, trusted proxy CIDRs, or a canonical HTTPS public origin are absent, or if either development bypass is enabled. Public origins cannot contain credentials, paths, queries, or fragments; database URLs must use a PostgreSQL protocol.
