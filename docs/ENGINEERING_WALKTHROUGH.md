...
It is not a commercial launch announcement. NuraPrep has a live 470-family owner-authorized Math bank: 38 individually owner-reviewed foundation versions and 432 deterministic expansion versions released under delegated exact-version screening. The Vercel application and scale-to-zero AWS Lambda mirror share a Neon database; Google sign-in, protected practice, and owner-only reviewer authorization have been exercised live. The product still lacks independent educator sampling, an externally validated score predictor, and qualified legal and manual-accessibility review. Stripe remains outside the career-fair MVP critical path, and the higher-cost ECS/RDS commercial AWS architecture remains validated but unapplied.
...
The Dockerfile builds a Next.js standalone server and copies only traced production output into a digest-pinned distroless Node.js image. The final runtime runs as unprivileged UID/GID 65532 and contains no shell or package manager. Its September 14, 2026 ECR enhanced scan completed with no reported findings, a point-in-time result that must be rechecked as vulnerability data changes. Compose supplies a one-shot migration service before the application starts and exposes a database-aware health route. The application pool has explicit connection, statement, client-query, idle-transaction, idle-connection, and connection-lifetime bounds; staging still needs to prove those values against its real task and RDS connection budgets.

The applied low-idle-cost Terraform root packages the same standalone application for Lambda, pins one ECR digest, loads an allowlisted encrypted SSM runtime parameter through exact-resource IAM, expires logs after seven days, and alarms on function errors. It omits provisioned concurrency, NAT, an ALB, Fargate, and RDS. A separate unapplied commercial Terraform target uses an HTTPS load balancer, private Fargate tasks, isolated encrypted PostgreSQL, Secrets Manager injection, a private source-artifact bucket, bounded logs, operational alarms, and an account budget. Production input guards require multiple application tasks, Multi-AZ RDS, deletion protection, a final snapshot, and one NAT gateway per availability zone. Those safeguards intentionally make production more expensive than the portfolio mirror.
...
- an applied commercial ECS/RDS AWS environment, RDS backup/restore drills, full production monitoring, or provider-calculator cost validation;
