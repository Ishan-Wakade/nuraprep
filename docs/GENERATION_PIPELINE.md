...
The registry now contains at least one bounded template for every Math leaf skill. Its 43 current templates declare a theoretical capacity of 848 before rejection: 400 numeric-response structures, 260 single-choice structures, 88 multiple-select structures, and 100 ordered-response structures. Internal difficulty capacity is 80 Foundational, 320 Developing, 368 Proficient, and 80 Advanced structures. Capacity is based on declared semantic context/frame combinations; parameter or number swaps do not increase it. The two reasoning-expansion passes add triangle area, a missing rectangle dimension, equivalent ratios, mixed-rational ordering, signed change, two-plan break-even equations, equivalent unit conversions, and event-likelihood ordering. Later depth passes add accessible bar-graph comparisons, direct evaluation of inequality solution sets, distributive-expression equivalence, reverse-percent word problems, grouped order of operations, proportional tables, composite area, and mixed-unit ordering. The calibration pass adds conceptual decimal place value and contextual perimeter at Foundational, plus scaled ratio/unit conversion and frequency-table weighted mean at Advanced. The public-outline depth pass adds round-then-estimate totals, direct table maxima, constant rates from paired values, and repeated circular-route distance. The algebra depth pass adds ordered expression evaluation, practical minimum inequalities, variables on both sides of an equation, and percent increase. Structured table and graph content participates in internal originality comparisons instead of leaving data questions to prompt-only signals. Each added template reaches its full 20-structure capacity under deterministic content, answer, math, misconception, and duplicate gates. Every family carries a deterministic answer proof. Twelve replay-protected pilots have staged 368 validated but unreviewed drafts: 24 from the first three-template pass, 96 from the full-taxonomy pass, 24 from a format-diversity pass, 32 from each of the two four-template reasoning expansions, 8 from the bar-graph pass, 8 from the inequality pass, 16 from the algebra/word-problem depth pass, 32 from the multi-step reasoning pass, 32 from the difficulty-calibration pass, 32 from the public-outline depth pass, and 32 from the algebra depth pass. The staged set contains 184 numeric, 104 single-choice, 40 multiple-select, and 40 ordered-response drafts; its internal difficulty labels comprise 32 Foundational, 136 Developing, 168 Proficient, and 32 Advanced candidates. Replaying every persisted recent batch created zero duplicates. This proves the generation, rejection, persistence, and replay-protection machinery; it does **not** mean thousands of questions are currently approved. Scaling responsibly requires multiple reasoning families and balanced response formats per skill, representative human sampling, and learner-performance calibration. Deterministic acceptance is an engineering gate, not human publication approval.
...
The reviewer queue links to `/review?sample=DETERMINISTIC_UNREVIEWED`, which selects one current unreviewed candidate from each deterministic template version. Selection uses the lowest stable prompt hash, with the version ID as a deterministic tie-breaker, so database row order cannot silently change the sample. The live 368-draft pilot currently produces 43 review items instead of asking an owner to inspect every numeric variation. Links preserve the sample context on the detail page, show the remaining exact-version count, and offer the next sampled draft without dropping the reviewer into the general queue.
