...
The registry now contains at least one bounded template for every Math leaf skill. Its 51 current templates declare a theoretical capacity of 1,008 before rejection: 440 numeric-response structures, 300 single-choice structures, 128 multiple-select structures, and 140 ordered-response structures. Internal difficulty capacity is 120 Foundational, 360 Developing, 408 Proficient, and 120 Advanced structures. Capacity is based on declared semantic context/frame combinations; parameter or number swaps do not increase it. The two reasoning-expansion passes add triangle area, a missing rectangle dimension, equivalent ratios, mixed-rational ordering, signed change, two-plan break-even equations, equivalent unit conversions, and event-likelihood ordering. Later depth passes add accessible bar-graph comparisons, direct evaluation of inequality solution sets, distributive-expression equivalence, reverse-percent word problems, grouped order of operations, proportional tables, composite area, and mixed-unit ordering. The calibration pass adds conceptual decimal place value and contextual perimeter at Foundational, plus scaled ratio/unit conversion and frequency-table weighted mean at Advanced. The public-outline depth pass adds round-then-estimate totals, direct table maxima, constant rates from paired values, and repeated circular-route distance. The algebra depth pass adds ordered expression evaluation, practical minimum inequalities, variables on both sides of an equation, and percent increase. The target-depth pass adds sum decomposition, table threshold selection, elapsed time, proportion-plus-conversion sequencing, percent of a whole, layered perimeter, median from a frequency table, and metric-conversion sequencing. Structured table and graph content participates in internal originality comparisons instead of leaving data questions to prompt-only signals. Each added template reaches its full 20-structure capacity under deterministic content, answer, math, misconception, and duplicate gates. Every family carries a deterministic answer proof. Thirteen replay-protected pilots have staged 432 validated but unreviewed drafts: 24 from the first three-template pass, 96 from the full-taxonomy pass, 24 from a format-diversity pass, 32 from each of the two four-template reasoning expansions, 8 from the bar-graph pass, 8 from the inequality pass, 16 from the algebra/word-problem depth pass, 32 from the multi-step reasoning pass, 32 from the difficulty-calibration pass, 32 from the public-outline depth pass, 32 from the algebra depth pass, and 64 from the balanced target-depth pass. The staged set contains 200 numeric, 120 single-choice, 56 multiple-select, and 56 ordered-response drafts; its internal difficulty labels comprise 48 Foundational, 152 Developing, 184 Proficient, and 48 Advanced candidates. Replaying every persisted recent batch created zero duplicates. This proves the generation, rejection, persistence, and replay-protection machinery; it does **not** mean 1,008 questions are currently approved. Scaling responsibly requires multiple reasoning families and balanced response formats per skill, representative human sampling, and learner-performance calibration. Deterministic acceptance is an engineering gate, not human publication approval.
...
The reviewer queue links to `/review?sample=DETERMINISTIC_UNREVIEWED`, which selects one current unreviewed candidate from each deterministic template version. Selection uses the lowest stable prompt hash, with the version ID as a deterministic tie-breaker, so database row order cannot silently change the sample. The live 432-draft pilot currently produces 51 review items instead of asking an owner to inspect every numeric variation. Links preserve the sample context on the detail page, show the remaining exact-version count, and offer the next sampled draft without dropping the reviewer into the general queue.

The broader `/review?sample=DETERMINISTIC_QUALITY_SAMPLE` queue unions four transparent sets: one anchor per template; a SHA-256-hash-ranked finite-population sample sized to a 95% detection target when at least 5% of the eligible population is assumed defective; one additional candidate from every template with Advanced, multiple-select, ordered-response, or table/graph content; and every eligible draft with a missing or malformed prompt hash. The calculation uses the exact without-replacement probability of missing every assumed defective item. The queue exposes its population, anchor, detection, risk, and missing-hash counts instead of hiding the policy in code.

The modeled probability assumes non-adversarial, uniformly distributed SHA-256 prompt hashes. It is a reproducible planning model, not independent random sampling, educational validation, approval inheritance, or evidence that sibling variants are correct. A decision applies only to the exact question version reviewed. Findings should be converted into template regression cases or controlled improvement proposals before another batch is generated; independent educator sampling and calibration against observed defect rates remain necessary before large-scale publication.
