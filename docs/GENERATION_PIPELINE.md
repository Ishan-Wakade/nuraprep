...
The registry now contains at least one bounded template for every Math leaf skill. Its 31 current templates declare a theoretical capacity of 608 before rejection: 260 numeric-response structures, 180 single-choice structures, 88 multiple-select structures, and 80 ordered-response structures. Capacity is based on declared semantic context/frame combinations; parameter or number swaps do not increase it. The two reasoning-expansion passes add triangle area, a missing rectangle dimension, equivalent ratios, mixed-rational ordering, signed change, two-plan break-even equations, equivalent unit conversions, and event-likelihood ordering. Later depth passes add accessible bar-graph comparisons, direct evaluation of inequality solution sets, distributive-expression equivalence, reverse-percent word problems, grouped order of operations, proportional tables, composite area, and mixed-unit ordering. Each of these templates reaches its full 20-structure capacity under deterministic content, answer, math, misconception, and duplicate gates. Every family carries a deterministic answer proof. Nine replay-protected pilots have staged 272 validated but unreviewed drafts: 24 from the first three-template pass, 96 from the full-taxonomy pass, 24 from a format-diversity pass, 32 from each of the two four-template reasoning expansions, 8 from the bar-graph pass, 8 from the inequality pass, 16 from the algebra/word-problem depth pass, and 32 from the multi-step reasoning pass. The staged set contains 128 numeric, 72 single-choice, 40 multiple-select, and 32 ordered-response drafts. Replaying every persisted recent batch created zero duplicates. This proves the generation, rejection, persistence, and replay-protection machinery; it does **not** mean thousands of questions are currently approved. Scaling responsibly requires multiple reasoning families and balanced response formats per skill, representative human sampling, and learner-performance calibration. Deterministic acceptance is an engineering gate, not human publication approval.
