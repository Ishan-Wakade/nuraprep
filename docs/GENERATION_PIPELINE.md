...
The registry now contains at least one bounded template for every Math leaf skill. Its 51 current templates declare a theoretical capacity of 1,008 before rejection: 440 numeric-response structures, 300 single-choice structures, 128 multiple-select structures, and 140 ordered-response structures. Internal difficulty capacity is 120 Foundational, 360 Developing, 408 Proficient, and 120 Advanced structures. Capacity is based on declared semantic context/frame combinations; parameter or number swaps do not increase it. The templates cover arithmetic, rational-number representations, ratios, conversions, expressions, equations, inequalities, word problems, measurement, geometry, data displays, probability, and statistics. Structured tables and graphs participate in internal originality comparisons instead of leaving data questions to prompt-only signals. Every family carries a deterministic answer proof.

The MVP release command reproducibly selects eight variants from every template and eight additional variants from three high-use templates, yielding 432 generated families. A regression scan prevents known article, repeated-adjective, capitalization, and singular/plural defects. The selected versions are staged, revalidated, linked to governed public-outline provenance, recorded under the owner-delegated release decision, and published alongside the 38-question reviewed foundation for 470 active Math families. Obsolete deterministic drafts are archived. The generated expansion is owner-authorized and machine-validated, not independently educator-reviewed or empirically calibrated. Deterministic acceptance and an owner release decision are engineering/product gates; they are not evidence that all 1,008 possible structures are approved or that difficulty labels predict real learner outcomes.
...
The reviewer queue can link to `/review?sample=DETERMINISTIC_UNREVIEWED`, which selects one current unreviewed candidate from each deterministic template version. Selection uses the lowest stable prompt hash, with the version ID as a deterministic tie-breaker, so database row order cannot silently change the sample. After the MVP release command, superseded pilot drafts are archived and the selected exact versions are active; future template versions return to the draft-only review workflow.
...
The pre-release 2026-09-11 audit passed all 432 staged drafts across 51 template versions and 12 leaf skills. The 2026-09-12 release regenerated the set after correcting 14 editorial defects, then passed the active-bank audit for all 470 published families. These audits establish schema, answer-contract, programmatic-math, provenance-policy, release-decision, and configured similarity gates. They cannot establish independent educational quality, accessibility in context, difficulty calibration, real-exam prediction, or legal originality.
